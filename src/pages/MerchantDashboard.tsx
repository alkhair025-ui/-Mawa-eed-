      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800">
            ...
            <div className="text-2xl font-black text-amber-300">{stats?.pendingCount || appointments.filter(a => a.status === 'pending').length}</div>
            <div className="text-[11px] text-amber-400 mt-1">بحاجة لمراجعة أو تأكيد</div>
          </div>
                  ...
                  <div key={app.id} className="p-4 bg-slate-950 rounded-2xl border border-slate-800/80 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-400 mt-1">
                          {app.service_title} • الموظف: <strong className="text-slate-200">{app.staff_name}</strong> • التاريخ: <strong className="text-slate-200">{app.appointment_date}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end border-t md:border-0 border-slate-800 pt-3 md:pt-0">
                      <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                        ...
                        {app.status === 'confirmed' ? 'مؤكد' : app.status === 'completed' ? 'مكتمل' : app.status === 'cancelled' ? 'ملغى' : 'قيد الانتظار'}
                      </span>
                    ...
                    <tr key={app.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <td className="py-3.5 px-2">
                        ...
                        >
                          ...
                          <option value="completed">تم الإنجاز (مكتمل)</option>
