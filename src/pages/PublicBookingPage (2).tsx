...
export default function PublicBookingPage() {
  ...
  useEffect(() => {
    const loadTaken = async () => {
      ...
      try {
        const res = await fetch(
          apiUrl(`/api/appointments?business_id=${business.id}&date=${selectedDate}`)
        );
        ...
        const taken = data
          .filter(
            (a: any) =>
              ...
              (!selectedStaff?.id || !a.staff_id || a.staff_id === selectedStaff.id)
          )
          .map((a: any) => a.appointment_time as string);
        setTakenSlots(taken);
      } catch {
        setTakenSlots([]);
      }
    ...
    loadTaken();
  }, [business?.id, selectedDate, selectedStaff?.id]);

  const submitBooking = async (asWaitlist = false) => {
    ...
    try {
      ...
      if (res.status === 409 || data.error === 'slot_taken') {
        ...
        setBookingError('هذا الوقت محجوز. يمكنك الانضمام لقائمة الانتظار أو اختيار وقت آخر.');
        // refresh taken slots
        setTakenSlots((prev) =>
      ...
      if (!res.ok) {
        setBookingError(data.message || data.error || 'حدث خطأ أثناء الحجز');
        return;
      }
      ...
      setBookingError('حدث خطأ أثناء إرسال الحجز، يرجى إعادة المحاولة.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <form onSubmit={handleCreateAppointment} className="bg-slate-900 rounded-3xl p-6 border border-slate-800 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                />
                {selectedDate && (
                  <div className="mt-2 px-3 py-2 rounded-xl bg-slate-950/80 border border-slate-800 text-[11px] text-slate-300 space-y-0.5">
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-500">ميلادي:</span>
                      <span className="font-semibold text-white">{formatDualDateFull(selectedDate).split(' · ')[0]}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-slate-500">هجري:</span>
                      <span className="font-semibold text-amber-400">{formatHijriAr(selectedDate)}</span>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-amber-500/40 bg-amber-500/10 p-4 space-y-3">
                ...
                {slotConflict && (
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-sky-600 hover:bg-sky-500 text-white font-bold text-xs transition"
                  >
                    {submitting ? 'جاري التسجيل...' : 'الانضمام لقائمة الانتظار على هذا الوقت'}
                  </button>
                )}
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-4 bg-gradient-to-r from-amber-500 via-amber-400 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:opacity-50 disabled:cursor-not-allowed text-slate-950 font-black text-sm rounded-2xl transition shadow-xl shadow-amber-500/20"
            >
        ...
        {step === 4 && confirmedBooking && (
          <div className="bg-slate-900 rounded-3xl p-8 border border-slate-800 text-center space-y-6 max-w-lg mx-auto animate-in zoom-in duration-300">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto border ${
              confirmedBooking.status === 'waitlist'
            ...
            <div>
              <span className={`text-xs font-bold px-3 py-1 rounded-full border ${
                ...
                {confirmedBooking.status === 'waitlist' ? 'تم إضافتك لقائمة الانتظار' : 'تم تأكيد موعدك بنجاح!'}
              </span>
              <h2 className="text-2xl font-black text-white mt-3">شكراً لك، {confirmedBooking.customer_name}</h2>
              <p className="text-xs text-slate-400 mt-1">
                {confirmedBooking.status === 'waitlist'
                  ? 'سنتواصل معك عند توفر الموعد. احتفظ برمز الحجز.'
                  : 'احتفظ برمز الحجز. يمكنك إرسال التأكيد عبر واتساب من الزر أدناه'}
              </p>
            </div>
            ...
            <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 text-right space-y-3 text-xs">
              ...
              <div className="flex justify-between border-b border-slate-800/80 pb-2 gap-3">
                <span className="text-slate-400 shrink-0">التاريخ والوقت:</span>
                <strong className="text-white text-left leading-relaxed">
                  {formatDualDateFull(confirmedBooking.appointment_date)}
                  <span className="block text-amber-400/90 text-[11px] mt-0.5">{confirmedBooking.appointment_time}</span>
                </strong>
              </div>
            ...
            <div className="pt-2 space-y-3">
              <p className="text-[11px] text-slate-400">
                اضغط الزر أدناه لفتح واتساب وإرسال رسالة التأكيد للعميل (أو انسخ الرسالة وأرسلها يدوياً).
              </p>
              <WhatsAppHelper
